...
(async () => {
  try {
    ...
    if (target) {
      ...
      STATUS_LOG.info("Connectable device found '%s' and validated.", CONNECTION_SELECTION);
    } else {
      STATUS_LOG.info(
        "Can't connect to device '%s' - no scan result for this mac address was found. Please check your network connection.",
