...
app.get("/devices/:deviceId/start", (req, res) => {
  ...
  if (!device) {
    ...
    (device as Hardware.IPObserver).powerOn();
...
const autostart = async () => {
  const serverDevice: Hardware.Device<Hardware.DeviceSettings> | undefined = deviceMap.get(2);
  const serverObserver: Hardware.IPObserver = serverDevice as Hardware.IPObserver;

  const clientDevice: Hardware.Device<Hardware.DeviceSettings> | undefined = deviceMap.get(3);
  const clientObserver: Hardware.IPObserver = clientDevice as Hardware.IPObserver;

  serverObserver.answerObserver = clientObserver;
  clientObserver.answerObserver = serverObserver;

  await serverObserver.powerOn();
  await clientObserver.powerOn();
};
